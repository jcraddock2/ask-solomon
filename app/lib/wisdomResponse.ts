...
export function getWisdomResponse(query: string): WisdomResponse | null {
  ...
  // --- DIFFICULT BOSS / TOXIC WORK RELATIONSHIP ---
  if (
    includesAny(q, [
      "my boss",
      "boss is",
      ...
      "hostile work",
      "toxic workplace",
      "someone at work treats",
      "coworker is mean",
    ])
  ) {
    return {
      emotionalState: "dealing with a difficult person in your work environment",
      deeperMeaning:
        "A difficult boss or toxic coworker is one of life's most draining challenges — because you cannot simply walk away. Wisdom does not ask you to absorb every wound. But it shows you how to respond with strength rather than react from hurt.",
      searchLanes: ["wisdom", "speech", "conflict", "peace", "leadership"],
      headline: "Solomon had something to say about difficult people",
      insight:
        "Proverbs says a soft answer turns away wrath — but it also says iron sharpens iron. Wisdom navigates difficult people shrewdly, not naively. You can respond with dignity without being a doormat.",
      reflection:
        "What would responding from wisdom look like here — instead of reacting from frustration?",
      nextStep:
        "Choose one way to respond differently in the next interaction — from strength, not fear or hurt.",
      bookConnection:
        "Success Secrets of Solomon — Conflict (pp. 138–140) and Relationships (pp. 130–134)",
    };
  }
